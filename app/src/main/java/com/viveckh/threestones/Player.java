...
public class Player {
	...
	//Temporary variable to receive result for tertiary operator
	...
	protected boolean PlaceAStone(char a_stone, int a_row, int a_column, Board a_board) {
		if (IsValidMove(a_stone, a_row, a_column, a_board)) {
			...
			if (a_board.SetStoneAtLocation(a_row, a_column, a_stone)) {
				...
				printStatus = printNotifications ? Notifications.Msg_MoveDescription(a_stone) : Notifications.Msg_NoMsg();
				return true;
			}
		}
		return false;
			...
			if (!a_board.IsLocationOccupied(a_row, a_column)) {
				...
				if (HasPermissionToOccupyVacantSpot(a_row, a_column, a_board)) {
					...
					if (IsStoneAvailable(a_stone)) {
						...
						printStatus = printNotifications ? Notifications.Msg_NoStonesToMove() : Notifications.Msg_NoMsg();
					}
				}
				else {
				...
				printStatus = printNotifications ? Notifications.Msg_AlreadyOccupied() : Notifications.Msg_NoMsg();
			}
		}
		else {
			printStatus = printNotifications ? Notifications.Msg_TapOnGameBoard() : Notifications.Msg_NoMsg();
		}
		return false;
	}
	...
	* */
	...
	public int CalculateScoreAfterMove(char a_stone, int a_placedInRow, int a_placedInColumn, Board a_board) {
		...
		if (IsLeftFavorable(a_placedInRow, a_placedInColumn, true, a_board) && IsRightFavorable(a_placedInRow, a_placedInColumn, true, a_board)) {
		...
		if (IsLeftFavorable(a_placedInRow, a_placedInColumn, true, a_board) && IsLeftFavorable(a_placedInRow, a_placedInColumn - 1, false, a_board)) {
		...
		if (IsRightFavorable(a_placedInRow, a_placedInColumn, true, a_board) && IsRightFavorable(a_placedInRow, a_placedInColumn + 1, false, a_board)) {
			if (a_board.GetStoneAtLocation(a_placedInRow, a_placedInColumn) == a_stone || a_board.GetStoneAtLocation(a_placedInRow, a_placedInColumn + 1) == a_stone || a_board.GetStoneAtLocation(a_placedInRow, a_placedInColumn  + 2) == a_stone) {
				points++;
		...
		if (IsBottomFavorable(a_placedInRow, a_placedInColumn, true, a_board) && IsBottomFavorable(a_placedInRow + 1, a_placedInColumn, false, a_board)) {
			if (a_board.GetStoneAtLocation(a_placedInRow, a_placedInColumn) == a_stone || a_board.GetStoneAtLocation(a_placedInRow + 1, a_placedInColumn) == a_stone || a_board.GetStoneAtLocation(a_placedInRow + 2, a_placedInColumn) == a_stone) {
				points++;
	...
	boolean IsLeftFavorable(int a_currentRow, int a_currentCol, boolean a_checkFarther, Board a_board) {
	...
	public boolean IsRightFavorable(int a_currentRow, int a_currentCol, boolean a_checkFarther, Board a_board) {
		Block current = new Block(a_board.GetBlockAtLocation(a_currentRow, a_currentCol));
		Block left = (a_currentCol > 0) ? new Block(a_board.GetBlockAtLocation(a_currentRow, a_currentCol - 1)) : null;
		Block right = (a_currentCol < a_board.GetBoardDimension() - 1) ? new Block(a_board.GetBlockAtLocation(a_currentRow, a_currentCol + 1)) : null;
	...
	//Refer to the description of isLeftFavorable
	public boolean IsTopFavorable(int a_currentRow, int a_currentCol, boolean a_checkFarther, Board a_board) {
		Block current = new Block(a_board.GetBlockAtLocation(a_currentRow, a_currentCol));
	...
	//Refer to the description of isLeftFavorable
	public boolean IsBottomFavorable(int a_currentRow, int a_currentCol, boolean a_checkFarther, Board a_board) {
		Block current = new Block(a_board.GetBlockAtLocation(a_currentRow, a_currentCol));
		Block top = (a_currentRow > 0) ? new Block(a_board.GetBlockAtLocation(a_currentRow - 1, a_currentCol)) : null;
		Block bottom = (a_currentRow < a_board.GetBoardDimension() - 1) ? new Block(a_board.GetBlockAtLocation(a_currentRow + 1, a_currentCol)) : null;
	...
	//Refer to the description of isLeftFavorable
	public boolean IsTopLeftFavorable(int a_currentRow, int a_currentCol, boolean a_checkFarther, Board a_board) {
		Block current = new Block(a_board.GetBlockAtLocation(a_currentRow, a_currentCol));
	...
	//Refer to the description of isLeftFavorable
	public boolean IsTopRightFavorable(int a_currentRow, int a_currentCol, boolean a_checkFarther, Board a_board) {
		Block current = new Block(a_board.GetBlockAtLocation(a_currentRow, a_currentCol));
	...
	//Refer to the description of isLeftFavorable
	public boolean IsBottomLeftFavorable(int a_currentRow, int a_currentCol, boolean a_checkFarther, Board a_board) {
		Block current = new Block(a_board.GetBlockAtLocation(a_currentRow, a_currentCol));
		Block topRight = (a_currentRow > 0 && a_currentCol < a_board.GetBoardDimension() - 1) ? new Block(a_board.GetBlockAtLocation(a_currentRow - 1, a_currentCol + 1)) : null;
		Block bottomLeft = (a_currentRow < a_board.GetBoardDimension() - 1 && a_currentCol > 0) ? new Block(a_board.GetBlockAtLocation(a_currentRow + 1, a_currentCol - 1)) : null;
